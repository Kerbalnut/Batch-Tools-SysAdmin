...
SET "_PowerShellFile=%~dp0\Enable-AdminShare.ps1" & REM %~dpn0.ps1 This script's (%0) [D]rive letter, [P]ath, and [N]ame, but with a .ps1 extension. E.g. HelloWorld.bat will launch HelloWorld.ps1
...
IF %_WindowsVersion% EQU 10 (
	...
	IF /I "%_ADMIN_OPTION%"=="RunAsAdministrator" (
		...
		PowerShell %_NO_PROFILE% -ExecutionPolicy %_ExecutionPolicy% -Command "& {Start-Process PowerShell -ArgumentList '%_NO_PROFILE% -ExecutionPolicy %_ExecutionPolicy% -File """"%_PowerShellFile%"""" %_POSH_PARAMS% ' -Verb RunAs}";
...
IF %_WindowsVersion% EQU 10 (
	...
	IF /I "%_ADMIN_OPTION%"=="RunAsAdministrator" (
		...
		PowerShell %_NO_PROFILE% -ExecutionPolicy %_ExecutionPolicy% -Command "& {Start-Process PowerShell -ArgumentList '%_NO_PROFILE% -ExecutionPolicy %_ExecutionPolicy% -File """"%_PowerShellFile%"""" %_POSH_PARAMS% -Verbose ' -Verb RunAs}";
...
IF %_WindowsVersion% EQU 10 (
	...
	IF /I "%_ADMIN_OPTION%"=="RunAsAdministrator" (
		PowerShell %_NO_PROFILE% -ExecutionPolicy %_ExecutionPolicy% -Command "& {Start-Process PowerShell -ArgumentList '%_NO_PROFILE% -ExecutionPolicy %_ExecutionPolicy% -File """"%_PowerShellFile%"""" %_POSH_PARAMS% -Debug ' -Verb RunAs}";
	)
) ELSE (
	IF /I "%_ADMIN_OPTION%"=="RunNonElevated" (
...
IF %_WindowsVersion% EQU 10 (
	...
	IF /I "%_ADMIN_OPTION%"=="RunAsAdministrator" (
		PowerShell %_NO_PROFILE% -ExecutionPolicy %_ExecutionPolicy% -Command "& {Start-Process PowerShell -ArgumentList '%_NO_PROFILE% -ExecutionPolicy %_ExecutionPolicy% -File """"%_PowerShellFile%"""" %_POSH_PARAMS% -Verbose -Debug ' -Verb RunAs}";
	)
) ELSE (
	IF /I "%_ADMIN_OPTION%"=="RunNonElevated" (
