 ...
 ; Test the script in regular Notepad to see if it works, or a blank version of the same application.
	...
	MsgBox Notepad++ Find-and-Replace function `n`nCtrl-Shift-X = to activate Part 1 of Find-and-Replace function. `nAlt-D = to activate Part 2 of Find-and-Replace function. ; A period is used to concatenate (join) two strings, and % is used to designate a expression. E.g.: MsgBox % "Kill repeat: " . killrepeat . "." And `n indicates a linefeed character. For breaking up even longer text, see MsgBox continuation section.
Return
